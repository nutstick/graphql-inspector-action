...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // config
        const config = yield loadConfig(tools, owner, repo);
        if (!config) {
...
function loadConfig(tools, owner, repo) {
    return __awaiter(this, void 0, void 0, function* () {
        const ref = process.env.GITHUB_SHA;
        const query = `
  ...
  `;
        ...
        try {
            const text = yield loadConfigFile({ ref, path: `${base}/${identifier}` });
            return js_yaml_1.safeLoad(text);
        }
        catch (e) {
            console.error(e);
            core.setFailed(`Failed to find ${base}/${identifier}.yaml or ${base}/${identifier}.yml file`);
        }
    });
}
function updateCheckRun(tools, { conclusion, output }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const response = yield tools.checks.listForRef(Object.assign({ ref: github.context.ref }, github.context.repo));
        ...
        if (!check && response.data.check_runs.length < 2) {
            ...
            yield tools.checks.create(Object.assign({}, github.context.repo, { name: checkName, head_sha: github.context.sha, status: 'in_progress', started_at: new Date().toISOString() }));
            const response = yield tools.checks.listForRef(Object.assign({ ref: github.context.ref }, github.context.repo));
        ...
        yield tools.checks.update(Object.assign({ check_run_id: check.id, completed_at: new Date().toISOString(), status: 'completed' }, github.context.repo, { conclusion,
            output }));
        // Fail
